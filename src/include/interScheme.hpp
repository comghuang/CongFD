...
constexpr real Teno5_Z2(std::array<real,5> q)
{
    ...
    for(int i=0;i<3;i++)
    {
        beta[i]=pow((C+tau/(beta[i]+eps)),qq);
        sumbeta+=beta[i];
    }
    ...
    case 5:
        /* 1,0,1 */
        // return beta[0]>beta[2]?(3.0/8.0*q[2]+3.0/4.0*q[3]-1.0 /8.0*q[4]):(3.0/8.0*q[0]-5.0/4.0*q[1]+15.0/8.0*q[2]);
        // break;
    case 4:
        /* 0,0,1 */
...
constexpr real Teno5_CongZ(std::array<real,5> q)
{
    ...
    std::array<real,3> beta;
    beta[0]= 1.0/1.0 *pow(1.0*q[0]-2.0*q[1]+1.0*q[2],2)
            + 1.0/4.0 *pow(1.0*q[0]-4.0*q[1]+3.0*q[2],2)+eps;

    beta[1]= 1.0/1.0  *pow(1.0*q[1]-2.0*q[2]+1.0*q[3],2)
            + 1.0/4.0 *pow(1.0*q[1]+0.0*q[2]-1.0*q[3],2)+eps;

    beta[2]= 1.0/1.0 *pow(1.0*q[2]-2.0*q[3]+1.0*q[4],2)
            + 1.0/4.0*pow(3.0*q[2]-4.0*q[3]+1.0*q[4],2)+eps;

    auto minBeta=std::min_element(beta.begin(),beta.end())-beta.begin();
    //if (beta[minBeta]<eps) return q[2];
    real tau=abs(beta[2]-beta[0]),KK=CTi*(beta[minBeta]+tau);//KK=CTi*(beta[minBeta]+tau);
    
    if (minBeta==0||(beta[0]+tau)*(beta[minBeta])>KK*beta[0])
    {
        flag+=1;
    }
    if (minBeta==1||(beta[1]+tau)*(beta[minBeta])>KK*beta[1])
    {
        flag+=2;
    }
    if (minBeta==2||(beta[2]+tau)*(beta[minBeta])>KK*beta[2])
    {
        flag+=4;
    }
...
constexpr real Teno5_CongZ2(std::array<real,5> q)
{
    ...
    for(int i=0;i<3;i++) 
    {
        if(minBeta==i||(beta[i]+tau)*beta[minBeta]+eps>KK*beta[i])
        {
            sumGamma+=gamma[i];
            result+=gamma[i]*(*u[i])(q[i],q[i+1],q[i+2]);
        }
