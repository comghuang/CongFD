...
inline std::array<real, 3> HLLCFlux1D(real rl, real ul, real pl, real rr,
                                      real ur, real pr) {
...
inline std::array<real, 4> HLLCFlux2D(real rl, real rr, real ul, real ur,
                                      real vl, real vr, real pl, real pr,
  ...
  real Sstar =
      ((pr - pl) + (rl * Vn[L] * (SL - Vn[L]) - rr * Vn[R] * (SR - Vn[R]))) /
  ...
  if (SL > 0) {
    ...
    for (int i = 0; i < 4; i++) {
      res[i] =
          (SL * (Sstar * U[i] + pStar * D[i]) - Sstar * F[i]) / (SL - Sstar);
    }
    ...
    for (int i = 0; i < 4; i++) {
      res[i] =
          (SR * (Sstar * U[i] + pStar * D[i]) - Sstar * F[i]) / (SR - Sstar);
    }
...
inline std::array<real, 4> HLLCFlux2D2(real rl, real rr, real ul, real ur,
                                       real vl, real vr, real pl, real pr,
                                       std::array<real, 3> norm) {
  enum { L, R };
  std::array<real, 4> res;
...
inline std::array<real, 4> roeFlux2D(real rl, real rr, real ul, real ur,
                                     real vl, real vr, real pl, real pr,
                                     std::array<real, 3> norm) {
  enum { L, R };
  ...
  FDispassion[1] +=
      lambda[1] * (coef2 * uAvg + rhoAvg * (deltaU - deltaVn * norm[0]));
  FDispassion[2] +=
      lambda[1] * (coef2 * vAvg + rhoAvg * (deltaV - deltaVn * norm[1]));
  FDispassion[3] +=
      lambda[1] * (coef2 * q_2Avg +
                   rhoAvg * (uAvg * deltaU + vAvg * deltaV - VnAvg * deltaVn));
...
inline std::array<real, 4> roeFlux2DSym(real rl, real rr, real ul, real ur,
                                        real vl, real vr, real pl, real pr,
                                        std::array<real, 3> norm) {
  enum { L, R };
  std::array<real, 4> FcL, FcR;
  ...
  FDispassion[0] += lambda[1] * (coef2 * 1.0 + rhoAvg * 0.0);
  FDispassion[1] +=
      lambda[1] * (coef2 * uAvg + rhoAvg * ((norm[1] > norm[0]) ? deltaU : 0));
  FDispassion[2] +=
      lambda[1] * (coef2 * vAvg + rhoAvg * ((norm[1] > norm[0]) ? 0 : deltaV));
  FDispassion[3] +=
      lambda[1] *
      (coef2 * q_2Avg +
       rhoAvg * ((norm[1] > norm[0]) ? uAvg * deltaU : vAvg * deltaV));
...
inline std::vector<real> fEuler2D(const std::vector<real> &q,
                                  std::array<real, 3> norm) {
  real r = q[0], u = q[1], v = q[2], p = q[3];
  real Vn = norm[0] > norm[1] ? u : v; //(u*norm[0]+v*norm[1]);
  // real H=GAMMA/(GAMMA-1)*p/r+(u*u+v*v)/2;
  std::vector<real> res = {r * Vn,
                           // u*Vn*r+p*norm[0],
                           // v*Vn*r+p*norm[1],
                           r * u * Vn + (norm[0] > norm[1] ? p : 0),
                           r * v * Vn + (norm[0] > norm[1] ? 0 : p),
                           ((u * u + v * v) / 2 * r + GAMMA / (GAMMA - 1) * p) *
                               Vn};
  return res;
}

inline std::vector<real> fEuler1D(const std::vector<real> &q,
                                  std::array<real, 3> norm) {
  real r = q[0], u = q[1], p = q[2];
  // real H=GAMMA/(GAMMA-1)*p/r+(u*u)/2;
  std::vector<real> res = {r * u, r * u * u + p,
                           (GAMMA / (GAMMA - 1) * p + r * (u * u) / 2) * u};
  return res;
}
inline std::vector<real> fDefault(const std::vector<real> &q,
                                  std::array<real, 3> norm) {
  return {q[0]};
}