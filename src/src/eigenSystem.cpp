...
eigensystemEuler2D::eigensystemEuler2D(const std::span<real, 4>& prim,
...
eigensystemEuler2D::eigensystemEuler2D(const std::span<real, 4>& priml,
...
std::array<real, 4>
eigensystemEuler2D::primToChar(const std::span<real, 4>& prim)
{
    ...
    res[0] = rut * leftEig[1] + rvt * leftEig[2] + ret * leftEig[3] + rt * leftEig[0];

    res[1] = rut * leftEig[5] + rvt * leftEig[6] + ret * leftEig[7] + rt * leftEig[4];

    res[2] = rut * leftEig[9] + rvt * leftEig[10] + ret * leftEig[11] + rt * leftEig[8];

    res[3] = rut * leftEig[13] + rvt * leftEig[14] + ret * leftEig[15] + rt * leftEig[12];

    return res;
}
std::array<real, 4>
eigensystemEuler2D::charToPrim(const std::span<real, 4>& chars)
{
    real ch1 = chars[0], ch2 = chars[1], ch3 = chars[2], ch4 = chars[3], rt, rut,
         rvt, ret;
    rt = ch3 * rightEig[2] + ch4 * rightEig[3] + ch2 * rightEig[1] + ch1 * rightEig[0];

    rut = ch3 * rightEig[6] + ch4 * rightEig[7] + ch2 * rightEig[5] + ch1 * rightEig[4];

    rvt = ch3 * rightEig[10] + ch4 * rightEig[11] + ch2 * rightEig[9] + ch1 * rightEig[8];

    ret = ch3 * rightEig[14] + ch4 * rightEig[15] + ch2 * rightEig[13] + ch1 * rightEig[12];
...
eigensystemEuler1D::eigensystemEuler1D(const std::span<real, 3>& priml,
    const std::span<real, 3>& primr, const std::array<real, 3>& norm_ = { 1, 0, 0 })
{
    gamma = GAMMA;
    enum { LL,
        RR };
...
eigensystemEuler1D::eigensystemEuler1D(const std::span<real, 3>& prim, const std::array<real, 3>& norm_ = { 1, 0, 0 })
{
    r = prim[0], u = prim[1], p = prim[2];
    gamma = GAMMA, ek = (u * u) / 2;
    h = p / r * gamma / (gamma - 1);
    c = std::sqrt(gamma * p / r);
    real ht = h + ek;
...
std::array<real, 3>
eigensystemEuler1D::primToChar(const std::span<real, 3>& prim)
{
...
std::array<real, 3>
eigensystemEuler1D::charToPrim(const std::span<real, 3>& chars)
{
