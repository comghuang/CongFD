...
SpaceDis::SpaceDis(int n_, Data *data_, Data *rhs_,
  ...
  case WCNS5Char:
    inter5 = &weno5_JSchen;
    inter5Positive = &weno5_JSchen;
    reconLMethod = (info->dim == 1) ? (&SpaceDis::reconLChar1D)
                                    : (&SpaceDis::reconLChar2D);
    reconRMethod = (info->dim == 1) ? (&SpaceDis::reconRChar1D)
                                    : (&SpaceDis::reconRChar2D);
    break;
  case WCNSZ5Char:
    inter5 = &weno5_Z;
    inter5Positive = &weno5_Z;
...
void SpaceDis::setConstNorm(std::array<real, 3> &&norm_) { norm = norm_; }
