...
void SpaceDis::calFluxEuler1D(int i)
{
    ...
    auto iflux=HLLCFlux1D(r,u,p,H,RT);
...
void SpaceDis::calFluxEuler2D(int i)
{
    ...
    arr2 r={reconL(i,0),reconR(i,0)};
    arr2 u={reconL(i,1),reconR(i,1)};
    arr2 v={reconL(i,2),reconR(i,2)};
    arr2 p={reconL(i,3),reconR(i,3)};
    arr2 H={reconL(i,4),reconR(i,4)};
    //std::vector<real> iflux=roeFlux1D2(r,u,p,H,RT);
    auto iflux=rhoFlux2D(r,u,v,p,H,norm);
    //std::vector<real> iflux2=roeFlux1D(r,u,p,H,RT);
