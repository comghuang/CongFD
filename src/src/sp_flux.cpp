...
void SpaceDis::calFluxEuler1D(int i) {
  ...
  auto iflux = roeFlux1D2(W[0], W[1], W[2], W[3], W[4], W[5]);
...
void SpaceDis::calFluxEuler2D(int i) {
  ...
  if (W[3] < 0 || W[0] < 0) {
    ...
    // std::array<real,5>
    // q={at(i-3,ivar),at(i-2,ivar),at(i-1,ivar),at(i,ivar),at(i+1,ivar)};
    ...
    W = {at(i - 1, 0), at(i - 1, 1), at(i - 1, 2), at(i - 1, 3),
         W[4],         W[5],         W[6],         W[7]};
  ...
  if (W[3 + 4] < 0 || W[0 + 4] < 0) //||isnan(WR[3])||isnan(WR[0]))
  {
    ...
    // beta[2]= 1.0/1.0 *pow(1.0*q[2]-2.0*q[3]+1.0*q[4],2)
    //     + 1.0/4.0*pow(3.0*q[2]-4.0*q[3]+1.0*q[4],2);
    // std::cout<<std::format("SpaceDis error R: negative pressure i={} WLR={}
    // WRR={} WLP={} WRP={} WS={:.4f} {:.4f} {:.4f} {:.4f} {:.4f} beta={:.4f}
    // {:.4f} {:.4f}\n"
  ...
  auto iflux =
      HLLCFlux2D2(W[0], W[4], W[1], W[5], W[2], W[6], W[3], W[7], norm);
  // std::vector<real> iflux2=roeFlux1D(r,u,p,H,RT);
