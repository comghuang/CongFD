...
void SpaceDis::difHCS() {
  ...
  std::vector<real> (*fFunction)(const std::vector<real> &,
                                 std::array<real, 3>);
...
void SpaceDis::difMND6() {
  ...
  for (int iNode = 0; iNode < n; iNode++) {
    ...
    for (int j = 0; j < nVar; j++)
      (*rhs)(i0 + iNode * offset, j) +=
          w[1] * (fluxNodeR[j] - fluxNodeL[j]) / h +
...
void SpaceDis::difTraditional6() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < nVar; j++) {
      (*rhs)(i0 + i * offset, j) +=
          75.0 / 64.0 * (fluxAt(i + 1, j) - fluxAt(i, j)) / h -
